...
const mockSubmission = {
  ...
  filePath: './storage/work-submissions/Alice_Dupont/Alice_Dupont_Rapport_final_attempt1.pdf',
  workId: 1,
  userId: 1,
  projectId: 42,
...
describe('WorkService', () => {
  ...
  describe('createWork', () => {
    ...
    it('should throw NotFoundException when module does not exist', async () => {
      ...
      await expect(service.createWork(dto as any)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should create tool and work in a transaction', async () => {
      ...
      const mockCreatedWork = { id: 1, maxAttempts: 3, toolId: 10, dueDate: new Date(dto.dueDate) };
      ...
      const result = await service.createWork(dto as any);
    ...
    it('should create work without dueDate when not provided', async () => {
      ...
      const mockTool = { id: 10, name: dto.name, type: 'WORK', moduleId: 5 };
      const mockCreatedWork = { id: 1, maxAttempts: 3, toolId: 10, dueDate: null };
  ...
  describe('updateWork', () => {
    it('should throw NotFoundException when work does not exist', async () => {
      ...
      await expect(service.updateWork(999, {}  as any)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw NotFoundException when new moduleId does not exist', async () => {
      ...
      await expect(service.updateWork(1, { moduleId: 99 } as any)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should update tool and work in a transaction', async () => {
      ...
      const result = await service.updateWork(1, { name: 'Nouveau nom', maxAttempts: 5 } as any);
  ...
  describe('submitWork', () => {
    ...
    it('should throw NotFoundException when work does not exist', async () => {
      ...
      await expect(service.submitWork(999, mockFile, mockReqUser)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw NotFoundException when user does not exist', async () => {
      ...
      await expect(service.submitWork(1, mockFile, mockReqUser)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should save file and create submission record', async () => {
      ...
      mockPrismaService.userWorkSubmission.create.mockResolvedValue(mockSubmission);
    ...
    it('should increment attempt number based on existing submissions', async () => {
      ...
      const createCall = mockPrismaService.userWorkSubmission.create.mock.calls[0][0];
  ...
  describe('removeSubmission', () => {
    it('should throw NotFoundException when submission does not exist', async () => {
      ...
      await expect(service.removeSubmission(999, 1)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw ForbiddenException when user does not own the submission', async () => {
      ...
      await expect(service.removeSubmission(1, 1)).rejects.toThrow(ForbiddenException);
    });
    ...
    it('should delete submission from DB and file from disk', async () => {
      ...
      mockPrismaService.userWorkSubmission.delete.mockResolvedValue(mockSubmission);
  ...
  describe('getLatestSubmission', () => {
    it('should return the most recent submission', async () => {
      mockPrismaService.userWorkSubmission.findFirst.mockResolvedValue(mockSubmission);

      const result = await service.getLatestSubmission(1, 1);

      expect(mockPrismaService.userWorkSubmission.findFirst).toHaveBeenCalledWith({
  ...
  describe('getUserSubmissions', () => {
    it('should throw NotFoundException when user does not exist', async () => {
      ...
      await expect(service.getUserSubmissions(999)).rejects.toThrow(NotFoundException);
    });

    it('should return all submissions with work and tool included', async () => {
      mockPrismaService.user.findUnique.mockResolvedValue(mockUser);
      mockPrismaService.userWorkSubmission.findMany.mockResolvedValue([mockSubmission]);

      const result = await service.getUserSubmissions(1);

      expect(mockPrismaService.userWorkSubmission.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { userId: 1 },
          orderBy: { submittedAt: 'desc' },
        })
